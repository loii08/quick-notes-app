...
import { 
  collection, query, orderBy, doc, updateDoc, deleteDoc, limit, startAfter, getDocs, QueryDocumentSnapshot, DocumentData 
            ...
            {filteredUsers.map(user => {
              ...
              return (
                <React.Fragment key={user.uid}>
                  <tr className="cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors" onClick={() => toggleExpand(user.uid)}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button className="text-sm text-gray-500 dark:text-gray-300">{isExpanded ? 'Show Less' : 'Show More'}</button>
                    </td>
                  </tr>
                  {isExpanded && (
                    <tr className="bg-gray-50 dark:bg-gray-700/20">
                      <td colSpan={3} className="px-6 py-4 space-y-2 text-sm text-gray-700 dark:text-gray-300">
                        ...
                        <div><strong>Notes Count:</strong> {user.notesCount ?? 0}</div>
                        <div><strong>Category Count:</strong> {user.categoryCount ?? 0}</div>
                        <div><strong>Quick Action Count:</strong> {user.quickActionCount ?? 0}</div>
        ...
        {filteredUsers.map(user => {
          ...
          return (
            <div key={user.uid} className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
              <div className="flex justify-between items-start cursor-pointer" onClick={() => toggleExpand(user.uid)}>
                <div>
              ...
              {isExpanded && (
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-600 space-y-2 text-sm text-gray-700 dark:text-gray-300">
                  <div><strong>Role:</strong> {user.role}</div>
                  <div><strong>Last Login:</strong> {formatDate(user.lastLogin)}</div>
                  <div><strong>Created On:</strong> {formatDate(user.createdAt)}</div>
                  <div><strong>Notes Count:</strong> {user.notesCount ?? 0}</div>
